...
it('has a valid composer.json with correct package name marko/cache-file', function () {
    $composerPath = dirname(__DIR__) . '/composer.json';

    expect(file_exists($composerPath))->toBeTrue()
        ->and(json_decode(file_get_contents($composerPath), true))->toBeArray()
        ->and(json_decode(file_get_contents($composerPath), true)['name'])->toBe('marko/cache-file');
});
...
it('has correct description in composer.json', function () {
    ...
    expect($composer['description'])->toBe('File-based cache driver for Marko Framework');
});
...
it('has PSR-4 autoloading configured for Marko\\Cache\\File namespace', function () {
    $composerPath = dirname(__DIR__) . '/composer.json';
    $composer = json_decode(file_get_contents($composerPath), true);

    expect($composer)->toHaveKey('autoload')
        ->and($composer['autoload'])->toHaveKey('psr-4')
        ->and($composer['autoload']['psr-4'])->toHaveKey('Marko\\Cache\\File\\')
        ->and($composer['autoload']['psr-4']['Marko\\Cache\\File\\'])->toBe('src/');
});
